...
def worker(
    ...
    for g_eid in g_eids:
        try:
            ...
            # #不取子问题中的最后一个/原始问题
            # question = ""
            # ques = data_item['generations'][0].split("\n")
            # if len(ques)>1:
            #     ques = ques[:-1]
            # for sub in ques:
            #     question = question + sub
            ori_question = data_item['question']
            ...
            print(f"Process#{pid}: Building prompt for eid#{g_eid}, original_id#{ori_question}")
...
def main():
    ...
    if args.dataset == 'nqtables':
        ...
        if args.dataset_split == 'dev':
            with open('data/NQ/ques/dev.json', 'r', encoding='utf-8') as f:
                dataset = json.load(f)
        if args.dataset_split == 'test':
            with open('data/NQ/ques/test.json', 'r', encoding='utf-8') as f:
                dataset = json.load(f)
    if args.dataset == 'ottqa':
        ...
        os.makedirs(args.save_dir, exist_ok=True)
        if args.dataset_split == 'dev':
            with open('data/OTT-QA/ques/dev_1000.json', 'r', encoding='utf-8') as f:
                dataset = json.load(f)
...
if __name__ == '__main__':
    ...
    # Prompt options
    parser.add_argument('--select_type', type=str, default='keyword',
