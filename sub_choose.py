...
def evaluate_subtable(sub_path):
    with open(sub_path,'r',encoding='utf-8') as f2:
        data = json.load(f2)
    acc = 0
    acc_table = 0
    data_len = len(data)
    for key in data:
        s_tables = data[key]["search_tables"]
        if args.dataset == 'ottqa':
            truth = data[key]["data_item"]["answer-text"].strip().lower()
        if args.dataset == 'nqtables':
            truth = data[key]["data_item"]["answer-text"][0].strip().lower()
        sub_text = []
        if len(s_tables) !=0:
            acc_table +=1
            for tab in s_tables:
                if len(s_tables[tab]) != 0:
                    for tab_text in s_tables[tab]["table"]:
                        sub_text.append(tab_text.strip().lower())
                    if "passage" in s_tables[tab]:
                        for ent in s_tables[tab]["passage"]:
                            e_text= s_tables[tab]["passage"][ent].strip().lower()
                            sub_text.append(e_text)
            for t in sub_text:
                if truth in t :
                    acc = acc+1
                    break
    print("数据总数为{}".format(data_len))
    print("存在黄金表格的个数为{}".format(acc_table))
    print("存在黄金答案的个数为{}".format(acc))
...
def extract_subtab(c_table, id):
    ...
    for t in table_text[2:]:
        split_t = t.split(" ")
        t_rowid = (split_t[0] + " "+ split_t[1]).strip()
        #t_rowid = t[:6].strip()
        if t_rowid in id :
            sub_tab.append(t)
...
def worker(
    ...
    for g_eid in g_eids:
        try:
            ...
            #临时追加
            with open('data/NQ/link/gpt_choose_nqtables_test.json', 'a', encoding='utf-8') as fw:
...
def main():
    ...
    if args.dataset == 'ottqa':
        ...
        if args.dataset_split == 'test':
            with open('data/OTT-QA/link/small_linear_ottqa_test.json', 'r', encoding='utf-8') as f:
            #with open('data/OTT-QA/link/test_5.json', 'r', encoding='utf-8') as f:
                dataset = json.load(f)
        if args.dataset_split == 'dev':
            with open('data/OTT-QA/link/small_linear_ottqa_dev.json', 'r', encoding='utf-8') as f:
                dataset = json.load(f)
    if args.dataset == 'nqtables':
...
if __name__ == '__main__':
    ...
    if args.dataset == 'ottqa':
        if args.dataset_split == 'test':
            ...
            #生成分解后的子表
            sub_table('data/OTT-QA/link/small_linear_ottqa_test.json','data/OTT-QA/link/result_choose_ottqa_test.json','data/OTT-QA/qa/subtable_ottqa_test.json')
        #if args.dataset_split == 'dev':
            #注意，不能用这个max_ans了，使用sub_choose2里的max_ans方法
            #max_ans("data/OTT-QA/link/gpt_choose_first.json","data/OTT-QA/link/result_choose_ottqa_dev.json")
            #生成分解后的子表
            #sub_table('data/OTT-QA/link/small_linear_ottqa_dev.json','data/OTT-QA/link/result_choose_ottqa_dev.json','data/OTT-QA/qa/subtable_ottqa_dev.json')

        #评估某表文件的答案召回率
        #evaluate_subtable('data/OTT-QA/link/linear_ottqa_dev.json')
        # evaluate_subtable('data/OTT-QA/link/small_linear_ottqa_dev.json')
        # evaluate_subtable('data/OTT-QA/qa/subtable_ottqa_dev.json')
        # evaluate_subtable('data/OTT-QA/qa/subtable_ottqa_dev2.json')

    if args.dataset == 'nqtables':
        # 注意，不能用这个max_ans了，使用sub_choose2里的max_ans方法
        #max_ans("data/NQ/link/gpt_choose_nqtables_dev.json","data/NQ/link/result_choose_nqtables_dev.json")
        #生成分解后的子表
        sub_table('data/NQ/link/small_linear_nqtables_test.json','data/NQ/link/result_choose_nqtables_test.json','data/NQ/qa/subtable_nqtables_test.json')
        #验证答案是否存在子表中
        evaluate_subtable('data/NQ/link/linear_nqtables_test.json')
        evaluate_subtable('data/NQ/link/small_linear_nqtables_test.json')
        evaluate_subtable('data/NQ/qa/subtable_nqtables_test.json')
        #evaluate_subtable('data/NQ/qa/subtable_nqtables_dev.json')
