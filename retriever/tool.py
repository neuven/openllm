...
class SimCSE(object):
        ...
        if use_faiss:
            ...
            if safe_index:
                if index_path is None:
                    logger.warning("no index path to save")
                else:
                    # 写到本地文件
                    faiss.write_index(index, index_path)  # 将索引，向量保存为index文件
    ...
    def read_index(self, sentences_or_file_path: Union[str, List[str]],
                    use_faiss: bool = True,
                    index_file: str = None,
                    device: str = None,):
        ...
        if use_faiss:
            quantizer = faiss.read_index(index_file)  # 读入index_file.index文件
            index = quantizer
            if (self.device == "cuda" and device != "cpu") or device == "cuda":
                if hasattr(faiss, "StandardGpuResources"):
                    logger.info("Use GPU-version faiss")
